...
#
# These are given as explicit (tile_url_template, attribution)
# pairs rather than folium's built-in name shortcuts (e.g.
# "CartoDB positron"). The named shortcuts are resolved through
# folium/branca's internal tile registry, and on some
# folium/branca versions that registry can point to a stale or
# mismatched URL template, which is what produces the
# "For development purposes only - API Key required" watermark
# even though no paid API is actually being used. Specifying the
# real, free, key-less tile URLs directly avoids that lookup
# entirely and guarantees no API key is ever required.
BASEMAP_OPTIONS = {
...
# ============================================================
# 5. Excel sheet names (used only when the matching CSV file
# is not found - see load_optional_table below).
# ============================================================
...
def normalize_identifier_columns(df):
    ...
    for col in id_columns:
        ...
        blank_mask = df[col].str.lower().isin(
            ["nan", "none", "null", "", "<na>"]
        )
...
def prepare_flood_gdf(flood_gdf):
    ...
    if DISTRICT_NAME_FIELD in gdf.columns:
        gdf[DISTRICT_NAME_FIELD] = (
            gdf[DISTRICT_NAME_FIELD].astype("string").str.strip()
        )

    if DISTRICT_ID_FIELD in gdf.columns:
        gdf[DISTRICT_ID_FIELD] = (
            gdf[DISTRICT_ID_FIELD].astype("string").str.strip()
        )
...
def robust_join_district_data(boundary_gdf, data_df):
    ...
    if (
        ...
        b = boundary_gdf.copy()
        b[DISTRICT_ID_FIELD] = b[DISTRICT_ID_FIELD].astype("string").str.strip()
        d = data_df.copy()
        d[DISTRICT_ID_FIELD] = d[DISTRICT_ID_FIELD].astype("string").str.strip()
    ...
    if (
        ...
        b = boundary_gdf.copy()
        b[DISTRICT_NAME_FIELD] = b[DISTRICT_NAME_FIELD].astype("string").str.strip()
        d = data_df.copy()
        d[DISTRICT_NAME_FIELD] = d[DISTRICT_NAME_FIELD].astype("string").str.strip()
...
def main():
    ...
    rivers_path = st.sidebar.text_input(
    ...
    population_df, population_err = safe_read_csv(population_path)
    ...
    district_df, district_err = load_optional_table(
        district_csv_path, excel_path, SHEET_DISTRICT_SITUATION
    )
    water_access_df, water_access_err = load_optional_table(
        water_access_csv_path, excel_path, SHEET_WATER_ACCESS
    )
    interventions_df, interventions_err = load_optional_table(
        interventions_csv_path, excel_path, SHEET_INTERVENTIONS
    )
    response_df, response_err = load_optional_table(
        response_csv_path, excel_path, SHEET_RESPONSE
    )
    response_plan_df, response_plan_err = load_optional_table(
        response_plan_csv_path, excel_path, SHEET_RESPONSE_PLAN
    )
    budget_df, budget_err = load_optional_table(
        budget_csv_path, excel_path, SHEET_BUDGET
    )
    facilities_df, facilities_err = load_optional_table(
        facilities_csv_path, excel_path, SHEET_FACILITIES
    )
    municipality_situation_df, municipality_situation_err = safe_read_csv(
        municipality_situation_path
    )
    event_info, event_info_err = load_event_info(event_info_path)
    markers_df, markers_err = safe_read_csv(markers_path)
    corridor_df, corridor_err = safe_read_csv(corridor_path)
    ...
    st.caption(
        "Select a District or Municipality on the map above to see its "
        "detailed situation and response plan below."
    )

    if district_field_source == "auto_generated":
        st.warning(
            "The District boundaries currently shown are temporary "
            "placeholders (Zone 1, Zone 2, ...) because flood.geojson has "
            "no real administrative District information. To connect "
            "District_Situation and other data automatically, add a real "
            "District name column to flood.geojson."
        )
    ...
    st.caption(
        f"Remaining population gap (all Districts): "
        f"{format_number(kpis['total_gap'])}"
    )
    ...
    with filter_col3:
        ...
        selected_label = st.selectbox(
            "Indicator shown on the map", indicator_labels, index=0
        )
    ...
    if not map_render_failed:
        try:
            map_state = st_folium(
                fmap, use_container_width=True, height=600, key="flood_map"
            )
        except Exception as e:
            st.error(
    ...
    st.caption(
        "Thick blue highlight = current selection. Dashed line = "
        "Municipality boundary. Use the layer control (top right of the "
        "map) to toggle Districts, Municipalities, rivers, and facilities."
    )
    ...
    if selected_municipality_name:
        ...
        st.info(
            ...
            "the 'Jump to a District / Municipality' filters, to see its "
...
def render_all_districts_table(merged, selected_column, selected_kind, selected_label):
    ...
    st.caption(
        "Values update automatically once district_situation.csv (or the "
        "District_Situation sheet) is filled in and Refresh data is "
